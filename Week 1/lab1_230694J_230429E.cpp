...
const unordered_set<string> keywords = {
    "let", "where", "true", "false", "not", "fn", "ls", "gr", "ge", "aug", 
    "le", "nil", "dummy", "or", "in", "eq", "ne", "and", "rec", "within"
};
...
public:
    ...
    Token scanToken() {
        ...
        // But since identifier checks isalpha() first, start with '_' is checked here? 
        // Wait, prompt says: Identifier -> Letter (Letter | Digit | '_')*
        // So Identifier MUST start with a letter.
    ...
    Token stringToken() {
        ...
        string text = ""; // RPAL strings usually include the quotes in raw value, but output example implies escaped content?
        // Prompt Output Example: <STRING, "whatever">
        // Prompt String definition: '''' ... '''' 
        // Let's store the content. The example output shows strings with double quotes in the value part??
        // Wait, the example output <IDENTIFIER, "x"> uses double quotes for value.
        // But the input has 'string'. 
        // Let's just capture the content.
        ...
        while (peek() != '\'' && !isAtEnd()) {
            ...
            if (c == '\\') {
                 advance(); // skip \ 
                 char next = peek();
                 if (next == 't') { text += "\\t"; advance(); }
                 else if (next == 'n') { text += "\\n"; advance(); }
                 else if (next == '\\') { text += "\\\\"; advance(); }
                 else if (next == '\'') { text += "\\'"; advance(); }
                 else {
                     // Unknown escape, just keep literal
                     text += '\\'; 
                     // Don't advance 'next' here, let loop handle it? 
        ...
        // So for a string 'abc', output should be <STRING, "abc">? Or <STRING, "'abc'">?
        // Usually scanners strip the quotes. The example output format wraps the VALUE in double quotes. 
        // <TYPE, "VALUE">.
...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        if (token.type == ERROR) {
            // Need to decide how to handle errors. Prompt says "Report lexical errors".
