...
struct DclnStorage *DclnTable = NULL;

int CurrentScope, LastDeclaration;

void InitializeDeclarationTable(void) {
  DclnTable = (struct DclnStorage *)malloc(sizeof(struct DclnStorage));
...
TreeNode Lookup(String S, TreeNode Source) {
  ...
  if (Element(DclnTable->CurrentDcln, S) == NullDeclaration) {
    ...
    Temp = Element(DclnTable->Dcln, Element(DclnTable->CurrentDcln, S));
...
void CloseScope(void) {
  ...
  for (i = CurrentScope + 1; i <= LastDeclaration; i++) {
    Assign(DclnTable->CurrentDcln, Element(DclnTable->Name, i),
